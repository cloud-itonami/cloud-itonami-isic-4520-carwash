(ns carwashops.operation
  "OperationActor -- one wash operation = one supervised actor run,
  expressed as a langgraph StateGraph. The advisor is sealed into a
  single node (`:advise`); its proposal is ALWAYS routed through the Car
  Wash Governor (`:govern`) and the rollout phase gate (`:decide`)
  ...
  :approved}}`. `:actuation/apply-wash-process` and
  `:actuation/return-vehicle` ALWAYS reach this node when the governor
  is clean -- see `carwashops.phase`.

  This shape is deliberately identical to every sibling actor's
  `operation.cljc`, because that identity is what lets the 営み OS
  adapter (`cloud_itonami/os/adapters/*.cljc`) be a three-line shim
  containing no judgement."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [carwashops.advisor :as advisor]
...
(defn build
  ...
  `carwashops.store/Store`).
  opts:
    :advisor      -- a `carwashops.advisor/Advisor` (default: mock-advisor)
